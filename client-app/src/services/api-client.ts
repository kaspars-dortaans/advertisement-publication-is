...
export class AdvertisementClient {
     ...
     * @return Success
     */
    getAdvertisementsByIds(body: number[] | undefined, cancelToken?: CancelToken): Promise<AdvertisementListItem[]> {
    ...
    protected processGetAdvertisementsByIds(response: AxiosResponse): Promise<AdvertisementListItem[]> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (const k in response.headers) {
        ...
        if (status === 200) {
            ...
            if (Array.isArray(resultData200)) {
                ...
                for (let item of resultData200)
                    result200!.push(AdvertisementListItem.fromJS(item));
            }
            else {
                result200 = <any>null;
            }
            return Promise.resolve<AdvertisementListItem[]>(result200);

        } else if (status !== 200 && status !== 204) {
            const _responseText = response.data;
            return throwException("An unexpected server error occurred.", status, _responseText, _headers);
        }
        return Promise.resolve<AdvertisementListItem[]>(null as any);
    }
