...
import { computed, type Ref } from 'vue'
  ...
  type FieldConfig<TValues, Path extends TPath<TValues>> = Partial<InputBindsConfig<TValue<TValues, Path>, TExtras>> | LazyInputBindsConfig<TValue<TValues, Path>, TExtras>
  ...
  export class Field<FieldType, TValues extends GenericObject> {
    ...
    constructor(path: TPath<TValues>, errors: ErrorsProperty<TValues>, model: Ref<FieldType>, attributes: Attributes){
        this.model = model;
...
export class FieldHelper<TValues extends GenericObject> {
  ...
  defineField(
    path: Path<TValues>,
    config?: FieldConfig<TValues, Path<TValues>>
  ) {
